...
SustAIn sends anonymous token-saved counts to our backend at:
...
SustAIn uses browser-local bundled model/runtime assets for prompt compression and a backend endpoint hosted on Cloudflare Workers for anonymous token-count logging.

## Data Retention

Local settings and totals remain on the user's device until cleared by the user or removed with the extension.

Anonymous token-count records sent to the backend may be retained for aggregate statistics.
